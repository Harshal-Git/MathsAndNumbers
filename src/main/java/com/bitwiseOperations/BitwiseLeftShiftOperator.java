/**
 * 
 */
package com.bitwiseOperations;

/**
 * @author Harshal-Git
 *
 * -> bitwise shift operations in java
 * 
 * << : left shift operator - shift the binary digits of given number to left : ignoring leading bits
 * 							- this results in given number multiplied with 2 with shift position as power 
 * 
 * -> it fills trailing bits with 0 for any type of number
 */
public class BitwiseLeftShiftOperator {

	/**
	 * @param args
	 */
	public static void main(String[] args) {

		int x = 3, y = 4, z = -1;
		
		// left shift operator
		
		// left shift digits by 1 place
		System.out.println("("+x+" << 1) = "+(x << 1)); // (3*(2^1) : shift position as power of 2)
		System.out.println("("+y+" << 1) = "+(y << 1)); // (4*(2^1))
		System.out.println("("+z+" << 1) = "+(z << 1));// (-1*(2^1))

		// left shift digits by 2 place
		System.out.println("("+x+" << 2) = "+(x << 2)); // (3*(2^2))
		System.out.println("("+y+" << 2) = "+(y << 2));// (4*(2^2))
		System.out.println("("+z+" << 2) = "+(z << 2));// (-1*(2^2))
		
	}

}
